"""
This is an automated way to upload multiple PFC test templates to CxAlloy.
...
def select_project():
    ...
    driver.find_element_by_xpath("//*[@id='checklist']/a").click()  # checklists

    time.sleep(1)
    driver.find_element_by_xpath("//*[@id='checklist-menu']/ul[1]/li[2]/a").click()  # checklist templates
...
def select_files():
    ...
    file_paths = list(filedialog.askopenfilenames(
        title="Select Prefunctional Checklist Templates for Upload"))
    return(file_paths)
...
def add_templates(file_name, file_path):
    ...
    driver.find_element_by_xpath("//*[@id='button-add-template']").click()  # add new
    time.sleep(1)
    driver.find_element_by_xpath("//*[@id='template-name']").send_keys(file_name)  # send title of test template
    #time.sleep(1)  # unneeded?
    driver.find_element_by_xpath("//*[@id='button-submit-add-template']").click()  # add
    time.sleep(1)
    driver.find_element_by_class_name('import-button').click()  # import

    fancyboxFrame = driver.find_element_by_class_name("fancybox-iframe")  # finding the fancybox popup
    driver.switch_to.frame(fancyboxFrame)  # switching the focus of selenium to the fancybox
    driver.find_element_by_class_name("checkbox-label").click()  # clicking checkbox for "background color to indicate headers"

    driver.find_element_by_name("file").send_keys(file_path)  # this file path for upload and will need to be generated and changed in a loop
    #
    driver.find_element_by_class_name("submit").click()  # continue
    # driver.find_element_by_class_name("submit").click()  # continue
    driver.find_element_by_class_name("import-select").send_keys("D")  # Import as description
    driver.find_element_by_class_name("submit").click()  # continue
    driver.find_element_by_class_name("submit").click()  # continue
    driver.find_element_by_class_name("submit").click()  # continue
